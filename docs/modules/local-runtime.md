...
提供唯一 Bun 后台、本地 HTTP 地图文档、CLI 入口和 launchd 生命周期。系统浏览器是
跨平台基线；macOS WKWebView 极薄壳只承载同一页面，不拥有业务状态或服务生命周期。
...
- `src/cli.ts`、`src/now.ts`：`serve`、`install`、`open`、`now`、`status` 等命令入口。
